...
with open('../output/rel2id.json',encoding='utf8') as f:
...
with open(path,encoding='utf8') as f:
    ...
    for index, each in enumerate(f):
            ...
            #这里其实是遇到了-DOCSTART-，长度为1
            if length<2:#说明在空行中

                # print('each_list',each_list)
                #中文的不需要空格，因为后面要分词；英文的要以空格隔开
                if True:#去除空格
                ...
                if len(each_sentence_entitys)>1:
                        ...
                        if each_sentence_entitys[1] not in all_ids_dict:
                            ...
                            all_ids_dict[each_sentence_entitys[1]] = each_sentence_entitys[1]
                ...
                if len(ll)<2:
                    ...
                    if ll[0]=='S':
                        ...
                        if eachs_entity not in each_sentence_entitys:
                            if True:#去除空格
                                eachs_entity=eachs_entity.replace(' ','')
...
with open(path_relation,'w',encoding='utf8') as f:
    #两种效果一样
    ## f.write(json.dumps(all_rel_data,indent=4))
    json.dump(all_rel_data,f,indent=4,ensure_ascii=False)
